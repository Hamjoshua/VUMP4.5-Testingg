...
echo "Compiling with kotlinx-cli from: $KOTLINX_CLI_JVM_JAR"

# Собираем все .kt файлы в массив (без tr!)
readarray -d '' KT_FILES < <(find src -name "*.kt" -print0)

echo "Compiling with user libs: ${KT_FILES[@]}"

# Компилируем — передаём массив напрямую
kotlinc -cp "$KOTLINX_CLI_JVM_JAR" "${KT_FILES[@]}" -include-runtime -d access-control.jar
